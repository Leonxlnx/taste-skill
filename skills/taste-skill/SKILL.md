...
Install commands, canonical doc links per design system, and the Apple Liquid Glass web approximation now live under `reference/` so the always-loaded body of this skill stays shorter. Read the file that matches what the brief needs; skip the rest.

- **`reference/install-commands.md`** - npm/yarn install commands per design system named in Section 2.
- **`reference/canonical-sources.md`** - canonical docs and GitHub links per design system. Read before reinventing a component that system already ships.
- **`reference/apple-liquid-glass.md`** - honest web approximation of Apple Liquid Glass (official vs unofficial, safer CSS skeleton). Only relevant when the brief asks for that aesthetic.
