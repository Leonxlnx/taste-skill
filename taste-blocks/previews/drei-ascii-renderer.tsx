...
function Shape(){const ref=React.useRef(null);const reduced=useReducedMotion();useFrame((_,d)=>{if(ref.current&&!reduced){ref.current.rotation.x+=d*.45;ref.current.rotation.y+=d*.6}});return <mesh ref={ref}><torusKnotGeometry args={[1, .35, 96, 12]}/><meshNormalMaterial/></mesh>}
